...
**Hatching Starts Below** and **Full Hatching Below** set the brightness range in Paper Style. Brightness is incoming light luminance plus ambient fill, clamped to 0–1; it is measured before paper tint. At or above the start, lighting adds no hatch ink. At or below the full threshold, it reaches **Shadow Ink**. Between them, the remapped brightness uses the existing **Form Ink** curve. For example, start **0.6** and full **0.2** keeps faces at 60% brightness and above clear, and reaches Shadow Ink at 20% and below. Defaults **1** and **0** preserve the original appearance. Equal thresholds make a hard cutoff that stays clear at equality; setting both to zero removes lighting-driven hatching. **Material Ink** remains an independent base deposit—set it to zero for completely clear faces. Thresholds do not alter lighting, paper tint, outlines or stroke placement.

Light Only displays unshadowed diffuse lighting, Raw Shadows displays diffuse-weighted shadow visibility, and Ink Demand displays the remapped tone before hatching (white means maximum ink).
