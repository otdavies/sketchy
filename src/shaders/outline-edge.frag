...
float depth(ivec2 q){return texelFetch(depthTexture,bounded(q),0).r*39.9+.1;}
float different(vec4 na,vec4 nb,float za,float zb,vec2 ds){
 ...
 float uncertainty=dot(abs(delta),vec3(1./255.))+39.9*2./16777215.;
 float normalEdge=step(.18,length(na.xyz-nb.xyz));
 float planeEdge=step(.008+worldPerPixel*.09+uncertainty,residual);
...
void main(){
 ...
 {vec2 ds=vec2(bounded(base+ivec2(1,0))-bounded(base+ivec2(0,0)))*.5*worldPerPixel;float e=different(m00,m10,z00,z10,ds);vec2 q=vec2(0.000,-0.250);sum+=e*q;moments+=e*vec3(q.x*q.x,q.x*q.y,q.y*q.y);count+=e;crossing+=e*vec2(1.,0.);}
 {vec2 ds=vec2(bounded(base+ivec2(0,1))-bounded(base+ivec2(0,0)))*.5*worldPerPixel;float e=different(m00,m01,z00,z01,ds);vec2 q=vec2(-0.250,0.000);sum+=e*q;moments+=e*vec3(q.x*q.x,q.x*q.y,q.y*q.y);count+=e;crossing+=e*vec2(0.,1.);}
 {vec2 ds=vec2(bounded(base+ivec2(2,0))-bounded(base+ivec2(1,0)))*.5*worldPerPixel;float e=different(m10,m20,z10,z20,ds);vec2 q=vec2(0.500,-0.250);sum+=e*q;moments+=e*vec3(q.x*q.x,q.x*q.y,q.y*q.y);count+=e;crossing+=e*vec2(1.,0.);}
 {vec2 ds=vec2(bounded(base+ivec2(1,1))-bounded(base+ivec2(1,0)))*.5*worldPerPixel;float e=different(m10,m11,z10,z11,ds);vec2 q=vec2(0.250,0.000);sum+=e*q;moments+=e*vec3(q.x*q.x,q.x*q.y,q.y*q.y);count+=e;crossing+=e*vec2(0.,1.);}
 {vec2 ds=vec2(bounded(base+ivec2(1,1))-bounded(base+ivec2(0,1)))*.5*worldPerPixel;float e=different(m01,m11,z01,z11,ds);vec2 q=vec2(0.000,0.250);sum+=e*q;moments+=e*vec3(q.x*q.x,q.x*q.y,q.y*q.y);count+=e;crossing+=e*vec2(1.,0.);}
 {vec2 ds=vec2(bounded(base+ivec2(0,2))-bounded(base+ivec2(0,1)))*.5*worldPerPixel;float e=different(m01,m02,z01,z02,ds);vec2 q=vec2(-0.250,0.500);sum+=e*q;moments+=e*vec3(q.x*q.x,q.x*q.y,q.y*q.y);count+=e;crossing+=e*vec2(0.,1.);}
 {vec2 ds=vec2(bounded(base+ivec2(2,1))-bounded(base+ivec2(1,1)))*.5*worldPerPixel;float e=different(m11,m21,z11,z21,ds);vec2 q=vec2(0.500,0.250);sum+=e*q;moments+=e*vec3(q.x*q.x,q.x*q.y,q.y*q.y);count+=e;crossing+=e*vec2(1.,0.);}
 {vec2 ds=vec2(bounded(base+ivec2(1,2))-bounded(base+ivec2(1,1)))*.5*worldPerPixel;float e=different(m11,m12,z11,z12,ds);vec2 q=vec2(0.250,0.500);sum+=e*q;moments+=e*vec3(q.x*q.x,q.x*q.y,q.y*q.y);count+=e;crossing+=e*vec2(0.,1.);}
