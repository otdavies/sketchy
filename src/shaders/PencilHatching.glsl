...
float pnNoise(vec2 p) {
    ...
    return mix(mix(pnHash(i),pnHash(i+vec2(1,0)),f.x),
               mix(pnHash(i+vec2(0,1)),pnHash(i+vec2(1,1)),f.x),f.y);
}
...
vec2 pnLineKey(float index,float level) {
    int j=int(abs(index));
    if(j==0)return vec2(0,-12);
    int count=0;
    ...
    return vec2(float(j)*sign(index),level-float(count));
}
...
float pnStroke(float across,float along,float level,float index,float width,
    ...
    vec2 key=pnLineKey(index,level)+vec2(familySeed*17.0,0);
    ...
    float segment=pnHash(key+vec2(cell,13.71));
    float birthRank=0.0001+0.9998*pnHash(key+vec2(cell+31.0,73.9));
    ...
    mask=mix(mask,0.73,smoothstep(0.25,0.8,fwidthT));
    float pressure=(0.62+0.50*r)*(0.72+0.28*sin(3.141593*f));
    float jitter=pnHash(key+vec2(frameSeed*0.719,91.0))-0.5;
    ...
    float newPressure=0.78+0.44*pnHash(key+vec2(frameSeed*0.33,cell+101.0));
    return core*mask*newPressure*exists;
}

float pnFamily(vec2 uv,vec2 dx,vec2 dy,float angle,float gap,float seed,float frameSeed) {
    vec2 a=vec2(cos(angle),sin(angle)),b=vec2(-a.y,a.x);
    float across=dot(uv,a)+seed*0.173;
    float along=dot(uv,b);
    float acrossRate=max(length(vec2(dot(dx,a),dot(dy,a))),1e-9);
    float alongRate=max(length(vec2(dot(dx,b),dot(dy,b))),1e-9);
    float lod=clamp(-log2(gap*acrossRate),-16.0,16.0);
    float level=floor(lod), sub=exp2(fract(lod)), birth=sub-1.0;
...
float pnChart(vec2 uv,vec2 dx,vec2 dy,float tone,float chartSeed,float frameSeed) {
    // A view-independent, smooth chart deformation: no latitude/longitude pole.
    vec2 q=uv+0.024*vec2(sin(uv.y*4.1+chartSeed),sin(uv.x*3.3-chartSeed));
    vec2 warpX=vec2(1.0,0.0792*cos(uv.x*3.3-chartSeed));
    vec2 warpY=vec2(0.0984*cos(uv.y*4.1+chartSeed),1.0);
    vec2 gx=warpX*dx.x+warpY*dx.y, gy=warpX*dy.x+warpY*dy.y;
...
float pnPencil(vec3 p,vec3 normal,vec3 dpdx,vec3 dpdy,float tone,float frameSeed) {
    ...
    float deposit=0.0;
    vec3 weights=pow(max(abs(normal)-vec3(0.22),vec3(0)),vec3(4));
    weights/=max(dot(weights,vec3(1)),1e-6);
#if (PN_CHART_MASK & 1)
